...
import { Rule, SchematicContext, Tree, apply, chain, externalSchematic, mergeWith, move, strings, template, url } from '@angular-devkit/schematics';
...
function updateModuleFile(
  ...
  const inserImportChange = insertImport(
    source,
    moduleFilePath,
  ...
  if(inserImportChange instanceof  InsertChange){
    updateRecorder.insertRight(
      inserImportChange.pos,
      inserImportChange.toAdd
    )
  }
